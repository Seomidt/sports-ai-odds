 ...
 * hanging regular-poller call can never block the backfill.
 * Uses Promise.race for timeout (works on all Node versions).
...
export async function fetchFixtureStatsForBackfill(fixtureId: number): Promise<ApiTeamStats[] | null> {
  ...
  const fetchAttempt = (async (): Promise<ApiTeamStats[] | null> => {
    try {
